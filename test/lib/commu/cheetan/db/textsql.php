...
class CTextDB
{
	...
	function insert_array( $data_array, $fpath = "" )
	{
		if( !$fpath )	$fpath = $this->fpath;
		if( $this->_read_header( $fpath, $maxid ) )
		{
			...
			foreach ($data_array as $key => $data) {
				...
				foreach( $this->columns as $i => $column )
				{
					if( $i )
					{
						$pieces[$column] = isset($data[$column]) ? $data[$column] : "";
					}
					else
					{
						$pieces[$column] = $maxid++;
				...
				$record_array[$key] = implode( TEXTDB_SEP, $pieces );
				$record_array[$key] = $this->_initstr($record_array[$key]);
				$pieces = null;
			}
